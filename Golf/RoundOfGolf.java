/**                                                       @version 2.1
   ...
   version 2.1: changed option 'u' to prompt user to re-enter scores or select a menu option.
                Change round par calculation logic.
 */
 ...
 */
   private static void playRound(GolfHole golfCourse[]) {
      ...
      for (int i = 0; i < golfCourse.length; i++) {
            ...
            while ((holeScore.matches("([1-9]|1[0])") == false) ||
...
/**
   ...
   String strokeType is the type of score(ace, eagle, etc). It is called to print to stdout later.
...
/**
   ...
   The parameter syntax is identical to parCalc.
 */   
 ...
 */   
   private static boolean listOptions(GolfHole golfCourse[]) {
      Scanner keyboard = new Scanner(System.in);
      int aces = 0, yourPar = 0, totalPar = 0, totalScore = 0;
      ...
      case "b"://b. Calculate and print the number of strokes above/below par for the round
         for (int i = 0; i < golfCourse.length; i++) {
             totalScore = golfCourse[i].getScore() + totalScore;
             totalPar = golfCourse[i].getPar() + totalPar;
             }//get total score and total par for the ENTIRE ROUND
          System.out.printf("\nYou scored %d this round. Even par for this course is %d.\n", totalScore, totalPar); 
          if (totalScore < totalPar) {
              yourPar = (totalPar - totalScore);
              System.out.printf("Your score is %d under par for the round.\n", yourPar);
          } else if (totalScore > totalPar) {
              yourPar = (totalScore - totalPar);
              System.out.printf("Your score is %d over par for the round.\n", yourPar);
          } else {
              System.out.printf("Your score is at even par for the round.\n");
          }
         System.out.printf("Press any key to continue.\n");         
         blocker = keyboard.nextLine();  
         break;
      ...
      case "u"://u. Select again
         System.out.printf("Are you sure you want to re-enter your scores?\nEnter 'u' to re-enter scores, or any other key to list options.\n");
         blocker = keyboard.nextLine().toLowerCase();
         if (blocker.equals("u")) {
 ...
 */
   public static void main(String[] args) {
      ...
      while (loopClosed == false) {
         loopClosed = listOptions(golfCourse);
      }
   }
}//end class 
