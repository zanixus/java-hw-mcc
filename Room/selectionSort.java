 ...
 */
   ...
   for (int i = 0; i < n - 1; i++) {
    ...
    public static void selectionSort(int roomCap[], int roomNum[]){         
        for (int i = 0; i < roomCap.length - 1; i++)
        {
            int index = i;
            for (int j = i + 1; j < roomCap.length; j++)
                if (roomCap[j] < roomCap[index]) 
                    index = j;      
            int smallerNum1 = roomCap[index];  
            int smallerNum2 = roomNum[index];
            roomCap[index] = roomCap[i];
            roomNum[index]  = roomNum[i];
            roomCap[i] = smallerNum1;
            roomNum[i] = smallerNum2;
        }
    }
 ...
 */      
      private static boolean bookRooms(Room[] reservations) {
         ...
         case "5": //List all available rooms sorted by capacity (low to high)
            ...
            for (int i = 0; i < arrayRoom.length; i++) {
            selectionSort(arrayCapacity, arrayRoom);
               if (arrayRoom[i] != 0 || arrayCapacity[i] !=0){
               System.out.println("Room: "+ arrayRoom[i] + " Capacity: " + arrayCapacity[i]);
               }
            }
